...
public class TriggerChecker<T>
{
    ...
    public void OnTriggerEnter(Collider other) => TryAddItem(other);

    public void OnTriggerStay(Collider other) => TryAddItem(other);
    ...
    {
        if (IsThisObject(collider.gameObject) == false) return;
        if (TryGetComponent(collider.gameObject, out var component) == false) return;
        if (_elements.Contains(component)) return;
        _elements.Add(component);
        _lastTransform = collider.gameObject.transform;
